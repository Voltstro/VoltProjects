...
//Modified to include yml/yaml. No one seems to include yml...
...
public static class MimeMap
{
    ...
    public static bool TryGetMimeType(string str, out string mimeType)
    {
        ...
        if (indexQuestionMark != -1) str = str.Remove(indexQuestionMark);


        if (!str.StartsWith(Dot))
        {
            int index = str.LastIndexOf(Dot);
            if (index != -1 && str.Length > index + 1) str = str.Substring(index + 1);
    ...
    public static string GetExtension(string mimeType, bool throwErrorIfNotFound = true)
    {
        ...
        if (mimeType.StartsWith(Dot)) throw new ArgumentException("Requested mime type is not valid: " + mimeType);

        if (_mappings.Value.TryGetValue(mimeType, out string extension)) return extension;
