# Script used for setting up default azurite container
...
def upload_file(container_client: ContainerClient, source: str, dest: str) -> None:
    ...
    #Try and get file mime type
    mime_type_guess = mimetypes.guess_type(source)
    mime_type = None
    if mime_type_guess is not None and len(mime_type_guess) >= 1:
    ...
    with open(source, "rb") as data:
        try:
            container_client.upload_blob(name=dest, data=data, content_settings=cnt_settings)
